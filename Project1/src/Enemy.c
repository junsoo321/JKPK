...
bool CanMove(float nextX, float nextY) {
    ...
    if (worldMap[gridY][gridX] == 1 || worldMap[gridY][gridRight] == 1 ||
        worldMap[gridBottom][gridX] == 1 || worldMap[gridBottom][gridRight] == 1) {
        return false;
    }
    return true;
}

// [수정] deltaTime 매개변수 추가
void UpdateAndDrawEnemies(SDL_Renderer* renderer, float playerX, float playerY, float deltaTime) {
    for (int i = 0; i < MAX_ENEMIES_PER_ROOM; i++) {
        if (!currentEnemies[i].active) continue;

        // 1. 이동 로직 (시간 기반)
        // 프레임마다 방향을 바꾸면 떨리므로, 아주 낮은 확률로만 방향 전환
        if (rand() % 1000 < 5) {
            currentEnemies[i].dirX = (float)((rand() % 3) - 1);
            currentEnemies[i].dirY = (float)((rand() % 3) - 1);
        }
        ...
        // 3. 출력 로직
        SDL_Rect enemyRect = { (int)currentEnemies[i].x, (int)currentEnemies[i].y, ENEMY_SIZE, ENEMY_SIZE };
